...
export const generatePdf = (payout) => {
  ...
  if (payout.status === PAYMENT_STATUS.PAID && payout.paidDate) {
    doc.text(`Payment Date: ${new Date(payout.paidDate).toLocaleDateString()}`, 20, 59);
  }
  ...
  doc.text(`- $${payout.platformFee.toFixed(2)}`, 170, yPos, { align: 'right' });
  yPos += 7;
  ...
  // Payment status
  yPos += 15;
  const statusText = `Payment Status: ${payout.status === PAYMENT_STATUS.PAID ? 'Paid' : payout.status === PAYMENT_STATUS.PENDING ? 'Pending' : 'Under Review'}`;
  doc.text(statusText, 105, yPos, { align: 'center' });
  
  // Footer
  doc.setFontSize(8);
  doc.text('This is an automatically generated receipt. For questions, please contact admin.', 105, 280, { align: 'center' });
...
export const generateCumulativePdf = (payouts, dateRange) => {
  ...
  // Add masaipay logo/header for cumulative report
  doc.setFontSize(24);
  // Add "masa" in black
  doc.setTextColor(0, 0, 0);
  doc.text('masa', 90, 20);
  ...
  yPos += 7;
  doc.text(`Total Platform Fee (5%): $${totals.platformFee.toFixed(2)}`, 20, yPos);
  yPos += 7;
  doc.text(`Total Net Amount: $${totals.netAmount.toFixed(2)}`, 20, yPos, { align: 'left' });
  ...
  // Table headers
  const headers = ['Date', 'Mentor', 'Gross', 'GST', 'Taxes', 'Fee', 'Net', 'Status'];
  ...
  payouts.forEach((payout, index) => {
    ...
    const email = payout.mentorEmail.length > 20 
  ...
  // Footer
  doc.setFontSize(8);
  doc.text('This is an automatically generated cumulative report. For questions, please contact admin.', 105, 280, { align: 'center' });
